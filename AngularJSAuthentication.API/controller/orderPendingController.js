'use strict';
app.controller('orderPendingController', ['$scope', 'OrderMasterService', 'OrderDetailsService', '$http', 'ngAuthSettings', '$filter', "ngTableParams", '$modal',
    function ($scope, OrderMasterService, OrderDetailsService, $http, ngAuthSettings, $filter, ngTableParams, $modal) {
        ...
        $scope.getData = function (pageno) {
            //// time cunsume code
            var url = serviceBase + "api/OrderPending" + "?list=" + $scope.itemsPerPage + "&page=" + pageno;
        ...
        $scope.searchdata = function (data) {
            ...
            var url = serviceBase + "api/SearchOrder?start=" + start + "&end=" + end + "&OrderId=" + $scope.srch.orderId + "&Skcode=" + $scope.srch.skcode + "&ShopName=" + $scope.srch.shopName + "&Mobile=" + $scope.srch.mobile + "&status=Pending";
            $http.get(url).success(function (response) {
        ...
        $scope.exportData = function (data) {
            ...
            var url = serviceBase + "api/SearchOrder?type=export&start=" + start + "&end=" + end + "&OrderId=" + $scope.srch.orderId + "&Skcode=" + $scope.srch.skcode + "&ShopName=" + $scope.srch.shopName + "&Mobile=" + $scope.srch.mobile + "&status=Pending";
            $http.get(url).success(function (response) {
                ...
                if ($scope.OrderByDate.length <= 0) {
                    ...
                    alasql('SELECT OrderId,Skcode,ShopName,RetailerName,Mobile,ItemID,ItemName,CategoryName,BrandName,SKU,Warehouse,Cluster,Date,Excecutive,MRP,UnitPrice,MOQPrice,Quantity,Discount,DiscountPercentage,Tax,TaxPercentage,TotalAmt,deliveryCharge,Status INTO XLSX("PendingOrderDetails.xlsx",{headers:true}) FROM ?', [$scope.NewExportData]);
                }
            });
        };
        //-------------------------------------------------------------------------//
