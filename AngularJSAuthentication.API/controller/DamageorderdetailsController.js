'use strict';
app.controller('DamageorderdetailsController', ['$scope', 'DamageOrderMasterService', 'DamageOrderDetailsService', '$http', '$window', '$timeout', 'ngAuthSettings', "ngTableParams", "peoplesService", '$modal', 'BillPramotionService', "$modalInstance", function ($scope, DamageOrderMasterService, DamageOrderDetailsService, $http, $window, $timeout, ngAuthSettings, ngTableParams, peoplesService, $modal, BillPramotionService, $modalInstance) {
    ...
    $scope.checkInDispatchedID = $scope.orderDetails[0].DamageOrderId;
    ...
    $scope.save = function () {
        ...
        for (var i = 0; i <= $scope.orderDetails.length; i++) {
            ...
            if (data.qty > data.CurrentStock && data.Deleted == false) {
...
app.controller("ModalInstanceCtrlOrderInvoice11", ["$scope", '$http', 'DamageOrderMasterService', "$modalInstance", 'ngAuthSettings', 'order',
